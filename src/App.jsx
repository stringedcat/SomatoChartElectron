...
const calculateEndomorphy = (Var14,Var16,Var17,Var3) => {
  ...
  Endomorphy = (-0.7182) + (0.1451 * PC) + (-0.00068 * Math.pow(PC, 2)) + (0.0000014 * Math.pow(PC, 3));
  setSomato(somato.push(Endomorphy));
}
const calculateMesomorphy = (Var5,Var14,Var8,Var19,Var12,Var13,Var3) => {
  let Mesomorphy = 0;
  let B = Var5 - (Var14 / 10);
  let P = Var8 - (Var19 / 10);
  Mesomorphy = 0.858 * Var12 + 0.601 * Var13 + 0.188 * B + 0.161 * P - Var3 * 0.131 + 4.5;
...
const calculateXY = (somato) => {
  ...
  const handleSubmit = (e) => {
        e.preventDefault();
    calculateEndomorphy(datosTemporales["Var14"],datosTemporales["Var16"],datosTemporales["Var17"], datosTemporales["Var3"]);

    calculateMesomorphy(datosTemporales["Var5"],datosTemporales["Var14"],datosTemporales["Var8"],datosTemporales["Var19"],datosTemporales["Var12"],datosTemporales["Var13"],datosTemporales["Var3"]);
  ...
  return (
    <div className='App'>
      ...
      <form onSubmit={handleSubmit}>
       ...
       <input type="text" /> Anónimo<input type="checkbox" name="" id="" /><br />
       ...
       <input type="number" name='Var3' onChange={ (e) => setDatosTemporales({...datosTemporales, Var3: parseFloat(e.target.value)})} /><br />
       <label htmlFor="" >Perímetro brazo (cm) </label>
       <input type="number" name='Var5' onChange={ (e) => setDatosTemporales({...datosTemporales, Var5: parseFloat(e.target.value)})}/><br />
       <label htmlFor="" >Perímetro de pierna (cm)</label>
       <input type="number" name='Var8' onChange={ (e) => setDatosTemporales({...datosTemporales, Var8: parseFloat(e.target.value)})}/><br />
       <label htmlFor="" >Diámetro humeral (cm) </label>
       <input type="number" name='Var12' onChange={ (e) => setDatosTemporales({...datosTemporales, Var12: parseFloat(e.target.value)})} /><br />
       <label htmlFor="" >Diámetro femoral (cm) </label>
       <input type="number" name='Var13' onChange={ (e) => setDatosTemporales({...datosTemporales, Var13: parseFloat(e.target.value)})} /><br />
       <label htmlFor="" >Pliegue tricipital (mm) </label>
       <input type="number"  name='Var14' onChange={ (e) => setDatosTemporales({...datosTemporales, Var14: parseFloat(e.target.value)})}/><br />
       <label htmlFor="" >Pliegue subescapular (mm)</label>
       <input type="number" name='Var16' onChange={ (e) => setDatosTemporales({...datosTemporales, Var16: parseFloat(e.target.value)})}/><br />
       <label htmlFor="" >Pliegue suprailíaco (mm)</label>
       <input type="number" name='Var17' onChange={ (e) => setDatosTemporales({...datosTemporales, Var17: parseFloat(e.target.value)})}/><br />
       <label htmlFor="" >Pliegue de pierna (mm) </label>
       <input type="number" name='Var19' onChange={ (e) => setDatosTemporales({...datosTemporales, Var19: parseFloat(e.target.value)})}/><br /><br />
